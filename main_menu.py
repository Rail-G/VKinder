...
for event in VKinder().bot_longpoll.listen():
    ...
    if event.type == VkBotEventType.MESSAGE_NEW:
        ...
        user_id = bot.user_name()[0]
        if text == '/start':
            bot.bot_write(user_id, f'Добро пожаловать {bot.user_name()}', Button.start())
        elif text in ['Начать поиск', 'Да, хочу начать сначала']:
            ...
            bot.bot_write(user_id, f'Пользователь {None} добавлен в черный список')
            pass
        elif text == 'Показать понравившиеся фотографии':
            bot.bot_write(user_id, 'Пользователи находящиеся в списке:')
            ...
            bot.bot_write(user_id, 'Пользователь удален из чёрного списка')
            pass
